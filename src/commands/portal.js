...
async function resolveEpicGroup(
  rawGroupPath,
  username,
  ...
  for (const candidate of candidates) {
    try {
      const epics = await fetchAssignedEpics(
        candidate,
        username,
        epicLabelFilter,
      );
...
async function fetchEpicIssues(group, epicIid) {
  try {
    ...
    const issues = await glabApi(
      `groups/${enc}/epics/${epicIid}/issues?per_page=50`,
    );
...
function printEpicIssues(issues, primaryBranches = {}) {
  ...
  for (const r of rows) {
    ...
    const titleSlice =
      r.title.length > 55 ? r.title.slice(0, 54) + "…" : r.title.padEnd(55);
    console.log(
      "  " +
        r.statusDot +
        " " +
...
async function cmdIssueView(issue, glabRepos = [], portalConfig = {}) {
  ...
  const labelLine =
    issue.labels?.length > 0
      ? "\n" +
        p.muted("  labels   ") +
        issue.labels.map((l) => colorLabel(l)).join(p.muted("  "))
      : "";
  console.log(
    boxen(
      p.muted("  project  ") +
        ...
        "\n" +
        p.muted("  title    ") +
        chalk.bold(p.white(issue.title)) +
        "\n" +
        p.muted("  state    ") +
        (issue.state === "opened" ? p.green("open") : p.muted("closed")) +
        labelLine +
        "\n" +
        p.muted("  url      ") +
        ...
        ? [
            {
              ...
              description: currentPrimary
                ? p.muted("current: ") + colorBranch(currentPrimary)
                : p.muted("none set"),
            },
          ]
        : []),
        ...
        ? [
            {
              ...
              description:
                p.muted("MR from ") +
                colorBranch(currentPrimary) +
                p.muted(" → default base branch"),
            },
          ]
        : []),
      {
  ...
  if (issueAction === "createBranch") {
    ...
    const { baseBranchName } = await enquirer.prompt({
      ...
      initial:
        projectDefaultBranch || portalConfig.defaultBaseBranch || "develop",
    ...
    try {
      ...
      console.log(
        boxen(
          chalk.bold(p.green("✔  Branch created")) +
            "  " +
            colorBranch(branchName.trim()),
          {
      ...
      if (!pb[configKey]) {
        ...
        saveConfig({
          ...cfgNow,
          portal: { ...cfgNow.portal, primaryBranches: pb },
        });
        console.log(
          "  " + p.teal("★") + "  " + p.muted("Set as primary branch"),
        );
      }
    } catch (e) {
      process.stdout.write(" ".repeat(40) + "\r");
  ...
  const primaryChoice = await search({
    ...
    source: (val) => {
      ...
      return [
        {
          value: "__cancel__",
          name: p.yellow("← Cancel"),
          description: p.muted("return to action menu without changing"),
        },
        {
          value: null,
          name: p.muted("— none —"),
        ...
        ...branches
          ...
          .map((b) => ({
            ...
            description:
              b.name === currentPrimary ? p.teal("★ current primary") : "",
          })),
      ];
    },
    theme: THEME,
  ...
  const updated = {
    ...(fresh.portal?.primaryBranches ?? {}),
    [configKey]: primaryChoice,
  };
  if (primaryChoice === null) delete updated[configKey];
  saveConfig({
    ...fresh,
    portal: { ...fresh.portal, primaryBranches: updated },
  });
...
async function createMrViaApi(
  projectPath,
  { sourceBranch, targetBranch, title, description, labels, isDraft },
) {
  const enc = encodeURIComponent(projectPath);
  const fields = {
...
async function cmdIssueMr(issue, glabRepos, portalConfig) {
  ...
  const configKey = issueConfigKey(issue);
  const primary =
    (loadConfig().portal?.primaryBranches ?? {})[configKey] ?? null;

  if (!primary) {
    console.log(
      "  " +
        p.yellow("No primary branch set.") +
        p.muted('  Use "Set primary branch" first.\n'),
    );
    return;
  }
  const localRepo = findLocalRepo(glabRepos, projectPath);
  if (!localRepo) {
    console.log(
      "  " +
        p.yellow("No local repo found for this issue.") +
        p.muted("  Is it within the current search scope?\n"),
    );
    return;
  }
  ...
  const pushFirst = await confirm({
    message: p.white("Push branch") + p.muted(" first?"),
    default: true,
    theme: THEME,
  });
  console.log();
  ...
  console.log(
    boxen(
      p.muted("  source   ") +
        colorBranch(primary) +
        p.muted(" → ") +
        colorBranch(targetBranch) +
        "\n" +
        p.muted("  title    ") +
        chalk.bold(p.white(title)) +
        "\n" +
        p.muted("  project  ") +
        p.white(localRepo.name) +
        (labels?.trim()
  ...
  const ok = await confirm({
    message: p.white("Create merge request?"),
    default: true,
    theme: THEME,
  });
  console.log();
  ...
  try {
    if (pushFirst) {
      ...
      await execFileAsync("git", ["push", "-u", "origin", primary], {
        cwd: localRepo.repo,
      }).catch(() => {});
    }
    const mr = await createMrViaApi(projectPath, {
    ...
    console.log(
      boxen(
        chalk.bold(p.green("✔  Merge request created")) +
          (url ? "\n  " + p.cyan(url) : ""),
        {
          padding: { top: 0, bottom: 0, left: 2, right: 2 },
          borderStyle: "round",
    ...
    if (url) {
      const openIt = await confirm({
        message: p.white("Open in browser?"),
        default: false,
        theme: THEME,
      });
      console.log();
    ...
    const isExisting =
      msg.includes("already exists") || msg.includes("open merge request");
    console.log(
      boxen(
        isExisting
...
async function cmdEpicMr(
  ...
  if (ready.length === 0) {
    console.log(
      "  " + p.muted("No issues ready. Set primary branches first.\n"),
    );
    return;
  }
  ...
  if (skipped.length > 0) {
    const maxSkip = Math.max(
      ...skipped.map(
        (c) => (c.localRepo?.name ?? c.projectPath.split("/").pop()).length,
      ),
      4,
    );
    for (const c of skipped) {
      const name = (c.localRepo?.name ?? c.projectPath.split("/").pop()).padEnd(
        maxSkip,
      );
  ...
  if (autoConfirm) {
    ...
    const choices = ready.map((c) => ({
      ...
      message:
        chalk.bold(p.white(c.localRepo.name)) + "  " + colorBranch(c.primary),
      hint: "",
      enabled: true, // pre-select all
    }));

    const selectedNames = await enquirer
      .autocomplete({
        name: "repos",
        ...
        footer() {
          return p.muted(
            "space to toggle · type to filter · enter to confirm · esc to go back",
          );
        },
        suggest(input = "", allChoices = []) {
          ...
          const filtered = term
            ? unselected.filter((c) => c.value.toLowerCase().includes(term))
            : unselected;
    ...
    console.log();
    if (
      selectedNames === "__back__" ||
      !Array.isArray(selectedNames) ||
      selectedNames.length === 0
    ) {
      if (selectedNames !== "__back__")
        console.log("  " + p.muted("Nothing selected.\n"));
      return;
    }
  ...
  const labels = autoConfirm
    ...
    : await input({
        ...
        theme: {
          ...THEME,
          style: { ...THEME.style, answer: (s) => p.purple(s) },
        },
      });
  ...
  const isDraft = autoConfirm
    ? false
    : await confirm({
        message: p.white("Mark all as") + p.muted(" Draft?"),
        default: false,
        theme: THEME,
      });
  ...
  const pushFirst = autoConfirm
    ? true
    : await confirm({
        message: p.white("Push branches") + p.muted(" first?"),
        default: true,
        theme: THEME,
      });
  ...
  const confirmed = autoConfirm
    ...
    : await confirm({
        message:
          p.white("Create ") +
          p.cyan(String(selectedReady.length)) +
          p.white(` MR${selectedReady.length !== 1 ? "s" : ""}?`),
        default: true,
        theme: THEME,
      });
  console.log();
  ...
  const listTasks = new Listr(
    selectedReady.map(({ issue, localIid, localRepo, primary }, i) => {
      const idx = p.muted(
        `[${String(i + 1).padStart(numWidth)}/${selectedReady.length}]`,
      );
      return {
        title:
          idx +
        ...
        task: async (_, task) => {
          try {
            ...
            if (
              ...
              msg.includes("open merge request")
            ) {
              results.push({
                name: localRepo.name,
                ok: true,
  ...
  const parts = [];
  if (opened.length)
    parts.push(chalk.bold(p.green(`✔  ${opened.length} opened`)));
  if (existing.length)
    parts.push(p.teal(`◉  ${existing.length} already existed`));
  ...
  console.log(
    boxen(
      parts.join(sep) +
        "\n" +
        p.muted(
          `${selectedReady.length} repo${selectedReady.length !== 1 ? "s" : ""} processed`,
        ),
      {
        padding: { top: 0, bottom: 0, left: 2, right: 2 },
        borderStyle: "round",
...
function findLocalRepo(glabRepos, projectPath) {
  ...
  return (
    glabRepos.find((r) => basename(r.repo).toLowerCase() === nameSegment) ??
    null
  );
}

// ── Editor detection ──────────────────────────────────────────────────────────
// Returns the first available editor CLI from a known list, or null if none
// found. Cursor, VS Code, and Windsurf all accept the same `<cmd> <file>` API.
function detectEditorCmd() {
  const candidates = ["cursor", "code", "windsurf", "agy"];
...
// ── Open as workspace ─────────────────────────────────────────────────────────
// Builds a .code-workspace file from the epic's local repos, saves it under
// .gitmux/workspaces/, and opens it in whichever editor (Cursor / VS Code /
// Windsurf) is available on PATH.
// terminal.integrated.cwd is set to process.cwd() so new terminals always
// open at the monorepo root rather than inside one of the workspace folders.
function cmdOpenAsWorkspace(epic, epicIssues, glabRepos) {
  ...
  for (const issue of epicIssues) {
    ...
    repoPaths.push(localRepo.repo);
  }

  if (repoPaths.length === 0) {
    console.log(
      "  " + p.muted("No local repositories found for this epic's issues."),
    );
    return;
  }

  const workspace = {
    folders: repoPaths.map((repoPath) => ({ path: repoPath })),
    settings: {
  ...
  const filePath = join(workspacesDir, `${slug}.code-workspace`);

  writeFileSync(filePath, JSON.stringify(workspace, null, 2) + "\n", "utf8");

  const repoList = repoPaths
    .map((r) => p.teal(basename(r)))
    .join(p.muted(", "));
  const editorCmd = detectEditorCmd();

  if (editorCmd) {
    // Spawn detached so the editor opens instantly without blocking the CLI.
    // shell:true ensures the shim is found regardless of how gitmux was invoked.
    ...
    console.log(
      "  " +
        p.green("✓") +
        " " +
        p.white(`Opening workspace in ${editorCmd}…`) +
        "\n" +
        "  " +
        p.muted("file   ") +
        p.cyan(filePath.replace(cwd + "/", "")) +
        "\n" +
        "  " +
        p.muted("repos  ") +
        repoList,
    ...
    // No known editor CLI found — print the file path so the user can open it.
    console.log(
      "  " +
        p.yellow("!") +
        " " +
        p.white("No editor CLI found (cursor / code / windsurf)") +
        "\n" +
        "  " +
        p.muted("file   ") +
        p.cyan(filePath.replace(cwd + "/", "")) +
...
async function cmdEpicCheckout(
  epicIssues,
  glabRepos,
  { autoConfirm = false } = {},
) {
  ...
  for (const issue of epicIssues) {
    ...
    const entry = {
      ...
      localIid,
      projectPath,
      configKey: issueConfigKey(issue),
    };
  ...
  if (tasks.length === 0) {
    console.log(
      "  " + p.muted("No local repos matched the issues in this epic.\n"),
    );
    return;
  }
  ...
  for (const { issue, localRepo, localIid, projectPath, configKey } of tasks) {
    ...
    if (!primary) {
      ...
      if (branches.length === 0) {
        console.log(
          "  " +
            p.muted("  No branches found for #" + localIid + " — skipping.\n"),
        );
        continue;
      }
          ...
          return [
            {
              value: null,
              name: p.muted("— skip —"),
              description: p.muted("don't checkout this repo"),
            },
            ...branches
              .filter((b) => !term || b.name.toLowerCase().includes(term))
              .map((b) => ({
      ...
      saveConfig({
        ...fresh,
        portal: { ...fresh.portal, primaryBranches: pb },
      });
    }
  ...
  const confirmed = autoConfirm
    ...
    : await confirm({
        message:
          p.white("Checkout ") +
          p.cyan(String(checkouts.length)) +
          p.white(" repo" + (checkouts.length !== 1 ? "s" : "") + "?"),
        default: true,
        theme: THEME,
      });
  console.log();
...
async function cmdEpicCrReview(epicIssues, glabRepos, portalConfig = {}) {
  ...
  if (ready.length === 0) {
    console.log(
      "  " +
        p.muted("No issues ready for review. Set primary branches first.\n"),
    );
    return;
  }
  ...
  const confirmed = await confirm({
    message:
      ...
      p.yellow("cr") +
      p.white(" review on ") +
      p.cyan(String(ready.length)) +
      p.white(` repo${ready.length !== 1 ? "s" : ""}?`),
    default: true,
    theme: THEME,
  });
  console.log();
  ...
  console.log(
    boxen(
      summaryParts.join(p.slate("   ·   ")) +
        "\n" +
        p.muted(
          `${ready.length} repo${ready.length !== 1 ? "s" : ""} reviewed`,
        ),
      {
        padding: { top: 0, bottom: 0, left: 2, right: 2 },
        borderStyle: "round",
...
export async function cmdPortal(
  ...
  try {
    ...
    const installLines = isMac
      ? p.muted("  brew     ") +
        p.cyan("brew install glab") +
        "\n" +
        p.muted("  MacPorts ") +
        p.cyan("sudo port install glab") +
        "\n" +
        p.muted("  asdf     ") +
        p.cyan("asdf plugin add glab && asdf install glab latest")
      : isWin
        ? p.muted("  winget   ") +
          p.cyan("winget install glab.glab") +
          "\n" +
          p.muted("  choco    ") +
          p.cyan("choco install glab") +
          "\n" +
          p.muted("  scoop    ") +
          p.cyan("scoop install glab") +
          "\n" +
          p.muted("  brew     ") +
          p.cyan("brew install glab") +
          p.muted("  (via WSL)")
        : isLinux
          ? p.muted("  brew     ") +
            ...
            p.muted("  snap     ") +
            p.cyan(
              "sudo snap install glab && sudo snap connect glab:ssh-keys",
            ) +
            "\n" +
            p.muted("  apt      ") +
            p.cyan("sudo apt install glab") +
            p.muted("  (WakeMeOps repo)") +
            "\n" +
            p.muted("  pacman   ") +
            p.cyan("pacman -S glab") +
            "\n" +
            p.muted("  dnf      ") +
            p.cyan("dnf install glab")
          : p.muted("  ") +
            p.cyan("https://gitlab.com/gitlab-org/cli#installation");
    ...
    console.log(
      boxen(
        chalk.bold(p.red("glab not found")) +
          ...
          p.white(
            platform
              ? `gitmux requires the GitLab CLI. Install it on ${platform}:\n\n`
          ...
          p.muted("  ") +
          p.cyan("glab auth login") +
          p.muted("   (follow the prompts to connect your GitLab account)") +
          "\n" +
          p.muted("  docs  ") +
          p.cyan("https://gitlab.com/gitlab-org/cli#installation"),
        {
  ...
  if (!group) {
    ...
    if (rawGroup) {
      ...
      group = await resolveEpicGroup(
        rawGroup,
        user.username,
        portalConfig.epicLabelFilter,
      );
      process.stdout.write(" ".repeat(40) + "\r");
      saveConfig({ ...config, portal: { ...portalConfig, group } });
      console.log(
        "  " + p.muted("Group auto-detected: ") + p.white(group) + "\n",
      );
    }
  }
  ...
  if (!group) {
    console.log(
      "  " +
        p.yellow("No GitLab group configured.") +
        p.muted("  Let's set it up.\n"),
    );
  ...
  try {
    epics = await fetchAssignedEpics(
      group,
      user.username,
      portalConfig.epicLabelFilter,
    );
  ...
  if (epics.length === 0 && group.includes("/")) {
    ...
    const betterGroup = await resolveEpicGroup(
      group,
      user.username,
      portalConfig.epicLabelFilter,
    );
    ...
    if (betterGroup !== group) {
      ...
      saveConfig({
        ...loadConfig(),
        portal: { ...(loadConfig().portal ?? {}), group },
      });
      try {
        epics = await fetchAssignedEpics(
          group,
  ...
  console.log(
    boxen(
      p.muted("user") +
        "  " +
        chalk.bold(p.white(user.username)) +
        "   " +
        p.muted("group") +
        "  " +
        chalk.bold(p.cyan(group)),
      {
        padding: { top: 0, bottom: 0, left: 2, right: 2 },
        borderStyle: "round",
        borderColor: "#334155",
  ...
  const hasCliAction =
    cliEpic || cliCheckout || cliCreateMr || cliCreateIssue || cliReview;
  if (hasCliAction) {
        ...
        console.log(
          boxen(
            chalk.bold(p.red(`Epic #${cliEpic} not found`)) +
              "\n" +
              p.muted("Available IIDs: ") +
              epics.map((e) => `#${e.iid}`).join(", "),
            {
              padding: { top: 0, bottom: 0, left: 2, right: 2 },
              borderStyle: "round",
              borderColor: "#f87171",
    ...
    if (resolvedEpic) {
          ...
          console.log(
            boxen(
              chalk.bold(p.yellow("CodeRabbit CLI not found")) +
                "\n\n" +
                p.muted("Install with: ") +
                p.cyan("curl -fsSL https://cli.coderabbit.ai/install.sh | sh") +
                "\n" +
                p.muted("Then authenticate: ") +
                p.cyan("cr auth login"),
              {
                padding: { top: 1, bottom: 1, left: 3, right: 3 },
                borderStyle: "round",
                borderColor: "#fbbf24",
      ...
      if (cliCreateMr) {
        ...
        return await cmdEpicMr(epicIssues, glabRepos, mergedPortalConfig, {
          autoConfirm,
        });
      }
      ...
      if (cliCreateIssue) {
        if (!cliIssueProject) {
          console.log(
            p.red(
              "  --issue-project is required for non-interactive issue creation.\n",
            ),
          );
          return 1;
        }
        if (!cliIssueTitle) {
          console.log(
            p.red(
              "  --issue-title is required for non-interactive issue creation.\n",
            ),
          );
          return 1;
        }
        ...
        const projectChoice = glabRepos.find(
          (r) =>
            r.name === cliIssueProject || r.projectPath === cliIssueProject,
        );
        if (!projectChoice) {
          console.log(
            p.red(
              `  Project "${cliIssueProject}" not found in local GitLab repos.\n`,
            ),
          );
          return 1;
        }
        ...
        let epicId = null;
        try {
          const epicRest = await glabApi(
            `groups/${encodeURIComponent(group)}/epics/${resolvedEpic.iid}`,
          );
        ...
        if (cliIssueDescription?.trim())
          issueFields.description = cliIssueDescription.trim();
        const effectiveLabels =
          cliIssueLabels ?? portalConfig.defaultLabels ?? null;
        if (effectiveLabels?.trim())
          issueFields.labels = effectiveLabels.trim();
        if (portalConfig.defaultMilestone?.id)
          issueFields.milestone_id = portalConfig.defaultMilestone.id;

        const enc = encodeURIComponent(projectChoice.projectPath);
        process.stdout.write("  " + p.muted("Creating issue…\r"));
        let issue;
        try {
          issue = await glabApi(`projects/${enc}/issues`, {
            method: "POST",
            fields: issueFields,
          });
          process.stdout.write(" ".repeat(40) + "\r");
        ...
        if (cliBranchName) {
          const baseBr =
            cliBaseBranch ?? portalConfig.defaultBaseBranch ?? "develop";
          process.stdout.write("  " + p.muted("Creating branch…\r"));
          try {
            await glabApi(`projects/${enc}/repository/branches`, {
              method: "POST",
            ...
            console.log(
              boxen(
                chalk.bold(p.green("✔  Branch created")) +
                  "  " +
                  colorBranch(cliBranchName.trim()),
                {
                  padding: { top: 0, bottom: 0, left: 2, right: 2 },
                  borderStyle: "round",
                  borderColor: "#4ade80",
            ...
            const cfgNow = loadConfig();
            const pbNow = {
              ...(cfgNow.portal?.primaryBranches ?? {}),
              [`${projectChoice.projectPath}#${issue.iid}`]:
                cliBranchName.trim(),
            };
            saveConfig({
              ...cfgNow,
              portal: { ...cfgNow.portal, primaryBranches: pbNow },
            });
          } catch (e) {
            process.stdout.write(" ".repeat(40) + "\r");
            console.log(
    ...
    if (resolvedEpic) {
      ...
      console.log(
        p.white(`  Epic #${resolvedEpic.iid}: `) +
          chalk.bold(p.cyan(resolvedEpic.title)) +
          "\n",
      );
  ...
  portalHome: while (true) {
      ...
      choices: [
        {
          value: "epics",
          name:
            chalk.hex("#FC6D26")("◈  Epics") +
            p.muted("   browse assigned epics, create issues & branches"),
          description: p.muted(
            "view epics, manage issues, checkout primary branches",
          ),
        },
        {
          value: "mr",
          name:
            p.purple("⎇  Merge Requests") +
            p.muted("   create MRs for local branches"),
    ...
    if (epics.length === 0) {
      console.log(
        boxen(
          p.yellow("No open epics assigned to you") +
            p.muted(" in ") +
            p.white(group),
          {
            padding: { top: 0, bottom: 0, left: 2, right: 2 },
    ...
    portalFlow: while (true) {
          ...
          return [
            {
              value: "__back__",
              name: p.yellow("← Go back"),
              description: p.muted("return to portal home"),
            },
            ...epics
              ...
              .map((e) => ({
                ...
                description:
                  ...
                  (e.labels.length > 0
                    ? "  " +
                      e.labels.map((l) => colorLabel(l)).join(p.muted("  "))
                    : ""),
              })),
          ];
        },
      ...
      issueLoop: while (true) {
          ...
          choices: [
              ...
              ? [
                  {
                    ...
                    description: p.muted(
                      "switch local repos to their primary branches",
                    ),
                  },
                ]
              : []),
              ...
              ? [
                  {
                    ...
                    description: p.muted(
                      "open a merge request per issue from each primary branch",
                    ),
                  },
                ]
              : []),
            ...(hasIssues && hasLocalHit
              ? [
                  {
                    value: "workspace",
                    name: p.white("⬡  Open as workspace"),
                    description: p.muted(
                      "create a VS Code workspace for this epic's local repos",
                    ),
                  },
                ]
              : []),
        ...
        if (action === "view") {
              ...
              return [
                {
                  value: "__back__",
                  name: p.yellow("← Go back"),
                  description: p.muted("return to action menu"),
                },
                ...epicIssues
                  .filter(
                    (i) =>
          ...
          console.log();
          if (issueChoice !== "__back__")
            await cmdIssueView(issueChoice, glabRepos, portalConfig);
          continue issueLoop;
        }
            ...
            return [
              {
                value: "__back__",
                name: p.yellow("← Go back"),
                description: p.muted("return to issue list"),
              },
              ...glabRepos
                .filter(
                  (r) =>
                ...
                .map((r) => ({
                  ...
                  description:
                    colorBranch(r.branch) + "  " + p.muted(r.projectPath),
                })),
            ];
          },
          theme: THEME,
        });
        if (projectChoice === "__back__") {
          console.log();
          continue issueLoop;
        }
        console.log();
        ...
        const previewLines = [
          p.muted("  epic        ") +
            chalk.bold(p.white(epic.title.slice(0, 55))),
          p.muted("  project     ") + chalk.bold(p.white(projectChoice.name)),
          p.muted("  title       ") +
            p.white(
              issueTitle.slice(0, 55) + (issueTitle.length > 55 ? "…" : ""),
            ),
          issueDesc.trim()
            ? p.muted("  description ") +
              p.white(
                issueDesc.slice(0, 55) + (issueDesc.length > 55 ? "…" : ""),
              )
            : null,
          issueLabels.trim()
          ...
          portalConfig.defaultMilestone?.title
            ? p.muted("  milestone   ") +
              p.teal(portalConfig.defaultMilestone.title)
            : null,
        ...
        if (portalConfig.defaultIteration?.id) {
          try {
            ...
            if (portalConfig.defaultIteration.id === "__current__") {
              const activeIters = await glabApi(
                `groups/${groupEnc}/iterations?state=current&per_page=1`,
              );
              iter = Array.isArray(activeIters) ? activeIters[0] : null;
              if (!iter || iter.due_date <= todayUtc) {
                const nextIters = await glabApi(
                  `groups/${groupEnc}/iterations?state=upcoming&per_page=1`,
                );
        ...
        try {
          const epicRest = await glabApi(
            `groups/${encodeURIComponent(group)}/epics/${epic.iid}`,
          );
          epicId = epicRest.id ?? null;
        } catch {
        ...
        if (wantBranch) {
          ...
          const { baseBranchName } = await enquirer.prompt({
            ...
            initial:
              ...
              portalConfig.defaultBaseBranch ||
              "develop",
            validate: (v) => v.trim() !== "" || "Base branch cannot be empty",
          });
          ...
          try {
            ...
            const pbNow = {
              ...(cfgNow.portal?.primaryBranches ?? {}),
              [`${projectChoice.projectPath}#${issue.iid}`]: branchName.trim(),
            };
            saveConfig({
              ...cfgNow,
              portal: { ...cfgNow.portal, primaryBranches: pbNow },
